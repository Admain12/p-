...
echo "${BOLD}${BLUE}---------------------------------------------------------------${RESET}"
echo
read -rp "  ${BOLD}Bond 接口名称${RESET} (默认 bond0): "
 BOND_NAMEBOND_NAME="${BOND_NAME:-bond0}"
echo
...
nmcli connection add type bond con-name "$BOND_NAME" ifname "$BOND_NAME" \ 
 ipv4.method "$IP_METHOD" ipv6.method ignore connection.autoconnect yes >/dev/null

nmcli connection modify "$BOND_NAME" bond.options "mode=$BOND_MODE,miimon=100"

if [ "$IP_METHOD" = "manual" ]; then
  echo "  配置静态 IP: ${YELLOW}$ADDR_CIDR${RESET}"
  nmcli connection modify "$BOND_NAME" ipv4.addresses "$ADDR_CIDR"
